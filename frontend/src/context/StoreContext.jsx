...
const normalizeUser=value=>value?{...value,addresses:Array.isArray(value.addresses)?value.addresses:[]}:null;
export function StoreProvider({children}){
  const[cart,setCart]=useState(()=>read('luma-cart-v2',[]));
  const[wishlist,setWishlist]=useState(()=>read('luma-wishlist',[]));
  const[user,setUserState]=useState(()=>normalizeUser(read('luma-user',null)));
  const[authLoading,setAuthLoading]=useState(()=>Boolean(localStorage.getItem('luma-token')));
  ...
  const setUser=value=>{const normalized=normalizeUser(value);setUserState(normalized);if(!normalized){setWishlist([]);setWishlistReady(true)}};
  ...
  const value=useMemo(()=>({cart,wishlist,user,setUser,authLoading,setAuthLoading,add,update,remove,clear:()=>setCart([]),toggleWish,wishlistReady,count:cart.reduce((s,x)=>s+x.quantity,0),subtotal:cart.reduce((s,x)=>s+x.product.price*x.quantity,0)}),[cart,wishlist,user,authLoading,wishlistReady]);
